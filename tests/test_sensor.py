"""Tests for Shopee Xpress sensor property logic."""
from datetime import datetime, timezone
from unittest.mock import MagicMock

from custom_components.shopee_xpress.const import ParcelStatus
from custom_components.shopee_xpress.sensor import (
    ShopeeXpressDeliveredParcelsSensor,
    ShopeeXpressIncomingParcelsSensor,
    ShopeeXpressLastUpdateSensor,
    ShopeeXpressNextDeliverySensor,
    ShopeeXpressParcelSensor,
)
...
def test_incoming_counts_and_lists():
    ...
    sensor = ShopeeXpressIncomingParcelsSensor(coordinator, _entry(), lambda _: None, set())
...
def test_parcel_sensor_status_and_attributes():
    ...
    sensor = ShopeeXpressParcelSensor(_coordinator([parcel]), _entry(), "A")
...
def test_parcel_sensor_missing_barcode():
    sensor = ShopeeXpressParcelSensor(_coordinator([_parcel("A")]), _entry(), "OTHER")
...
def test_next_delivery_picks_earliest():
    ...
    sensor = ShopeeXpressNextDeliverySensor(coordinator, _entry())
...
def test_next_delivery_none_without_moments():
    sensor = ShopeeXpressNextDeliverySensor(_coordinator([_parcel("A")]), _entry())
    assert sensor.native_value is None
    assert sensor.extra_state_attributes == {}
...
def test_delivered_sensor():
    ...
    sensor = ShopeeXpressDeliveredParcelsSensor(coordinator, _entry())
...
def test_last_update_sensor():
    ...
    sensor = ShopeeXpressLastUpdateSensor(coordinator, _entry())
