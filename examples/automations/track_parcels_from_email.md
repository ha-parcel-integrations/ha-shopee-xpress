# Automatic parcel tracking from e-mail (IMAP → Shopee Xpress)

Companion guide for [`track_parcels_from_email.yaml`](track_parcels_from_email.yaml): watch your mailbox(es) for shipping e-mails, extract the Shopee Xpress tracking code, and register it with `shopee_xpress.track_parcel` — fully automatic, no extra custom component required.

Shopee Xpress is a **code-based** carrier: it has no account inbox, so every parcel must be registered by its tracking code before the integration can follow it. This recipe automates exactly that step.

**How it works, in one line:** the core [IMAP integration](https://www.home-assistant.io/integrations/imap/) fires an `imap_content` event for every new e-mail (including the body); the automation extracts the tracking code — a cheap regex first, an optional AI fallback for everything else — and calls `shopee_xpress.track_parcel`.

```
new e-mail ──imap_content──▶ automation ──▶ regex match? ──▶ shopee_xpress.track_parcel
                                     │
...
- This integration, with the `shopee_xpress.track_parcel` action available (field `tracking_code`).
...
```
(?<![0-9A-Za-z])(?:BR|MY|TH|SPEPH|SPXID|SPXVN)[0-9A-Za-z]{8,}
```

Six prefixes are covered — one per market this integration currently supports: **`BR…`**, **`MY…`**, **`TH…`** (2 letters) and **`SPEPH…`** (Philippines), **`SPXID…`** (Indonesia), **`SPXVN…`** (Vietnam) (5 letters). The integration itself never rejects a tracking code — it warns, once, on anything matching neither the known alphanumeric shape nor a plain digit string, since a stricter check has already been wrong twice as new markets shipped new prefix lengths. Matching that broadly against e-mail text would hit every phone number and order line in the mail, so this example anchors on the known prefixes instead. **If your market carries a different prefix, add it to the alternation** (`(?:BR|MY|TH|SPEPH|SPXID|SPXVN|YOURPREFIX)`); the AI fallback will surface unknown formats in the meantime. A pure-digit order number is a valid Shopee Xpress input too, but is deliberately not matched by this regex — it looks like any other number in the mail body, so let the AI fallback (or a manual add) handle that case.
...
1. **Jinja eats backslashes in string literals.** A template stored as `regex_findall('\bBR…')` silently becomes a **backspace character** (`\b` is a string escape), so the regex never matches — no error anywhere. That's why the pattern is backslash-free: `(?<![0-9A-Za-z])` lookarounds instead of `\b`, `[0-9]` instead of `\d`. Copy that style if you add a prefix.
...
curl -X POST -H "Authorization: Bearer $HA_TOKEN" -H "Content-Type: application/json" \
  ...
  -d '{"sender":"noreply@shopee.com","subject":"Your parcel is on its way",
       "text":"Tracking code: MY000000000000","initial":true,"folder":"INBOX","username":"test"}'
```

Then `shopee_xpress.untrack_parcel` the test code afterwards. For a full end-to-end test, forward a real shipping mail to the watched mailbox — it must arrive **unread**.
