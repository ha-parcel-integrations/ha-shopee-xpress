"""Coordinator for the Shopee Xpress parcel tracker integration.
...
from .api import ShopeeXpressApiClient, ShopeeXpressApiError
from .const import (
...
def _refresh_interval(entry: ConfigEntry) -> timedelta:
    ...
    Not user-configurable on purpose: rate limiting on this endpoint has
    never been measured and there is no batch form, so poll cost is one
    request per registered parcel — letting users dial the cadence down would
    multiply that against an unmeasured host.
    """
    return timedelta(minutes=REFRESH_INTERVAL_MINUTES)


class ShopeeXpressCoordinator(DataUpdateCoordinator[list[dict]]):
    ...
    tracking codes the user entered (stored in the entry options), all
    resolved against the one market this hub is bound to. Each is fetched
    ...
    def __init__(
        ...
        # There is no "not found" placeholder here (unlike most carriers in
        # this suite): the failure envelope this endpoint returns cannot be
        # told apart from a genuinely broken backend, so a parcel with no
        # cache yet is simply not shown until a fetch actually succeeds.
    ...
    async def _async_update_data(self) -> list[dict]:
        ...
        market = self.config_entry.options.get(CONF_MARKET)
        codes = self._tracked()
        ...
        for code, result in zip(codes, results):
            if isinstance(result, BaseException):
                if not isinstance(
                    result, (ShopeeXpressApiError, aiohttp.ClientError)
                ):
                    raise result
                errors += 1
                _LOGGER.warning("Shopee Xpress fetch failed for %s: %s", code, result)
                cached = self._raw_cache.get(code)
                if cached is not None:
                    raws.append(cached)
                continue

            # The requested code is what normalize_parcel() falls back to for
            # `barcode` when `order_info` is absent — stamp it on the raw dict
            # rather than threading a second argument through every helper.
            result[REQUESTED_CODE_KEY] = code
            self._raw_cache[code] = result
            raws.append(result)

        if codes and errors == len(codes) and not raws:
            raise UpdateFailed("Shopee Xpress unreachable for all tracked parcels")

        include_history = self._include_history
        normalized = [
            normalize_parcel(raw, market=market, include_history=include_history)
            for raw in raws
        ]
