"""Sensor platform for the Shopee Xpress parcel tracker integration."""
...
async def async_setup_entry(
    hass: HomeAssistant,
    entry: ShopeeXpressConfigEntry,
    async_add_entities: AddEntitiesCallback,
) -> None:
    """Set up Shopee Xpress sensor entities from a config entry."""
    ...
    entities: list[SensorEntity] = [
        ShopeeXpressIncomingParcelsSensor(
            coordinator, entry, async_add_entities, current_barcodes
        ),
    ]
    for parcel in coordinator.data or []:
        entities.append(
            ShopeeXpressParcelSensor(coordinator, entry, parcel.get("barcode", ""))
        )
    entities.append(ShopeeXpressNextDeliverySensor(coordinator, entry))
    entities.append(ShopeeXpressDeliveredParcelsSensor(coordinator, entry))
    entities.append(ShopeeXpressLastUpdateSensor(coordinator, entry))

    async_add_entities(entities)


class ShopeeXpressIncomingParcelsSensor(
    CoordinatorEntity[ShopeeXpressCoordinator], SensorEntity
):
    ...
    def _handle_coordinator_update(self) -> None:
        ...
        if new_barcodes:
            self._async_add_entities(
                ShopeeXpressParcelSensor(self.coordinator, self._entry, barcode)
...
class ShopeeXpressParcelSensor(CoordinatorEntity[ShopeeXpressCoordinator], SensorEntity):
    """Per-parcel sensor reporting the status of one tracked Shopee Xpress parcel."""
    ...
    def __init__(
        self, coordinator: ShopeeXpressCoordinator, entry: ConfigEntry, barcode: str
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
        self._entry = entry
...
class ShopeeXpressNextDeliverySensor(
    CoordinatorEntity[ShopeeXpressCoordinator], SensorEntity
):
    ...
    def __init__(
        self, coordinator: ShopeeXpressCoordinator, entry: ConfigEntry
    ) -> None:
        """Initialize the sensor."""
        super().__init__(coordinator)
...
class ShopeeXpressDeliveredParcelsSensor(
    CoordinatorEntity[ShopeeXpressCoordinator], SensorEntity
):
    """Recently delivered tracked Shopee Xpress parcels."""
...
class ShopeeXpressLastUpdateSensor(
    CoordinatorEntity[ShopeeXpressCoordinator], SensorEntity
):
    """Diagnostic sensor reporting when Shopee Xpress was last polled successfully."""
